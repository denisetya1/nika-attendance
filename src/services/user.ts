...
import { forgotPasswordSchema, loginSchema, registerShcema, resetPasswordSchema, tokenSchema } from "@/lib/zod/user";
...
export const forgotPassword = async (prevState: unknown, formData: FormData) => {
  const validateFields = forgotPasswordSchema.safeParse(Object.fromEntries(formData.entries()));
...
export const verifyToken = async (prevState: unknown, formData: FormData) => {
  const validateFields = tokenSchema.safeParse(Object.fromEntries(formData.entries()));
